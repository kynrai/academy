...
This exercises MongoDB's flexible schema and its query operators (`$exists`, `$gt`, `$size`, `$expr`). Look up the correct operator for each case in the MongoDB documentation. For "more than two frameworks", use `$expr` with `$size`, e.g. `{ $expr: { $gt: [ { $size: "$frameworks" }, 2 ] } }` (prefer this over `$where`, which runs server-side JavaScript and is often disabled).
