---
title: "Day 4 – MongoDB: Document Databases"
---
...
Setup: `go get go.mongodb.org/mongo-driver/v2/mongo`
...
- Research the correct MongoDB query operator to find documents whose `frameworks` array contains more than one entry. The `$size` operator matches arrays of an exact length, but for "greater than", you need a different approach. Prefer `$expr` with `$size`, e.g. `{ $expr: { $gt: [ { $size: "$frameworks" }, 1 ] } }`. (Avoid `$where`: it runs arbitrary server-side JavaScript and is slow and often disabled.) Write the query that works.
...
- [mongo-go-driver v2 documentation](https://pkg.go.dev/go.mongodb.org/mongo-driver/v2/mongo)
