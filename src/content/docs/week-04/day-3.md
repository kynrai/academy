---
title: "Day 3 – PostgreSQL: Production Relational Databases"
---
...
conn = psycopg.connect("postgresql://academy_user:password@localhost:5432/academy")
...
Setup: `uv add "psycopg[binary]"`

Note: `%s` is the placeholder syntax for psycopg (not `?` as in SQLite). `ON CONFLICT (name) DO NOTHING` is PostgreSQL's equivalent of SQLite's `INSERT OR IGNORE`.
