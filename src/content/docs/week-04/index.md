...
**PostgreSQL** — macOS: `brew install postgresql@18 && brew services start postgresql@18`. Linux: `sudo apt install postgresql && sudo service postgresql start`.
...
**Redis** — macOS: `brew install redis && brew services start redis`. Linux: `sudo apt install redis-server && sudo service redis-server start`. Note: Redis relicensed to AGPLv3 starting with Redis 8 (2025), so many distributions now ship Valkey — a BSD-licensed, wire-compatible drop-in fork — instead; all client code in this course works unchanged with either.
