---
title: "Day 1 – Docker Images and Dockerfiles"
---
...
- Pull and explore an image: `docker pull python:3.13-slim`. Run an interactive shell: `docker run -it python:3.13-slim bash`. Install a package inside, exit, and confirm it is gone after the container stops. Explain what this demonstrates about container ephemeral state.
- Write a `Dockerfile` for your Python CLI utility from Week 3. Start with `FROM python:3.13-slim`, copy your source, install dependencies with `pip install --no-cache-dir -r requirements.txt`, and set a `CMD`. Build it: `docker build -t week3-cli:latest .`.
