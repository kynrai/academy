---
title: "Day 2 – Multi-Stage Builds and Security"
---
...
## Tasks
- Write a multi-stage Dockerfile for your TypeScript/Node.js backend from Week 4: Stage 1 (`FROM node:24 AS builder`) installs all deps and runs `npm run build`. Stage 2 (`FROM node:24-alpine AS runtime`) copies only the compiled `dist/` and production `node_modules`. Measure the image size difference: `docker images`.
