---
title: "Day 5 – Image Optimisation and Hardening"
---
...
## Tasks
- Audit your current images with `docker scout quickview` (or `trivy image your-image:latest` if Trivy is installed). Count the number of CVEs. Switch your base image from `python:3.13` to `python:3.13-slim` or `gcr.io/distroless/python3` and rescan. Record the reduction in vulnerabilities.
...
- Pin your base image to a specific digest for deterministic builds: `FROM python:3.13-slim@sha256:<digest>`. Get the digest with `docker inspect python:3.13-slim | grep Id`. Explain why using `:latest` is risky in production.
