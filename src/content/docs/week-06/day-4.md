---
title: "Day 4 – Generics and Type Safety"
---
...
```python
# Python 3.12+: PEP 695 inline type parameters (the recommended style).
# On older versions, use the TypeVar/Generic form (still valid) instead.
...
```python
# constrained to int or float (PEP 695 syntax, Python 3.12+)
def total[N: (int, float)](items: list[N]) -> N:
    return sum(items)
```
...
```go
// Go 1.21+: cmp.Ordered is in the standard library
import "cmp"
